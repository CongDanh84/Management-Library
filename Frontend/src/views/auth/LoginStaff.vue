...
const form = ref({ MSNV: '', Password: '' })
const loading = ref(false)
const errorMsg = ref('')

async function handleSubmit() {
  errorMsg.value = ''
  if (!form.value.MSNV || !form.value.Password) {
    errorMsg.value = 'Vui lòng nhập đầy đủ mã số nhân viên và mật khẩu.'
    return
  }
  loading.value = true
  try {
    await auth.loginStaff(form.value)
    ...
    <div class="card auth-card">
      ...
      <form @submit.prevent="handleSubmit">
        <div class="field">
          ...
          <input id="msnv" v-model="form.MSNV" placeholder="NV0001" autocomplete="username" />
        </div>
        <div class="field">
      ...
      <p class="auth-switch">
        <RouterLink :to="{ name: ROUTE_NAMES.LOGIN_READER }">Đăng nhập với tư cách độc giả</RouterLink>
      </p>
    </div>
  </div>
</template>
